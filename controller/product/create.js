angular
  ...
  .controller(
    ...
    function (
      ...
      // define variable
	  var countdownDuration = 60000; // Countdown duration in milliseconds
	  var countdownInterval = 1000; // Countdown interval in milliseconds
	  self.countdown = Math.floor(countdownDuration / countdownInterval);
  
	  function startCountdown() {
		var countdownTimer = $timeout(function () {
		  self.countdown--;
      ...
      if (is_new) {
        ...
        request
          .getAllCategoryByProductId(root.getCurProductId())
          .then(function (resp) {
        ...
        request
          .getAllProductImageByProductId(root.getCurProductId())
          .then(function (resp) {
            for (var i = 0; i < 4; i++) {
              self.images_obj[i] =
                resp.data[i] == null || resp.data[i] == ''
                  ? null
                  : resp.data[i];
            }
          });
      ...
      self.onFileSelect = function (files) {
        ...
        $mdToast.show(
          $mdToast.simple().content('Selected file').position('bottom right')
        );
      };
        ...
        if (is_new) {
          ...
          request.insertOneProduct(p).then(function (resp) {
            ...
            if (resp.status == 'success') {
              ...
              request
                .insertAllProductCategory(self.product_category)
                .then(function () {
                  self.done_arr[0] = true;
                }); // insert table relation
              request
                .uploadFileToUrl(self.image.file, dir, p.image, '')
                .then(function () {
          ...
          p.image =
            self.image.file != null
              ? now + root.getExtension(self.image.file)
              : p.image;
          request.updateOneProduct(p.id, p).then(function (resp) {
            ...
            if (resp.status == 'success') {
              ...
              if (self.image.file != null) {
                request
                  .uploadFileToUrl(self.image.file, dir, p.image, oldname)
                  .then(function () {
                    self.done_arr[1] = true;
                  }); // upload primary image
              } else {
      ...
      self.$watchCollection('done_arr', function (new_val, old_val) {
        if (self.submit_done || (new_val[0] && new_val[1] && new_val[2])) {
          loop_run = false;
          $timeout(function () {
            // give delay for good UI
            if (self.resp_submit.status == 'success') {
              if (self.send_notif) {
                self.sendNotification(
                  is_new ? self.resp_submit.data : self.resp_submit.data.product
                );
              }
              root.showConfirmDialogSimple(
                '',
                self.resp_submit.msg,
                function () {
      ...
      self.isReadySubmit = function () {
        if (is_new) {
          ...
          self.is_clean =
            angular.equals(original, self.product) &&
            angular.equals(self.original_category, self.category_selected) &&
            angular.equals(self.original_images, self.images);
          if (self.image.file != null) {
            return (
              self.category_valid && self.image.valid && self.isImagesValid()
            );
          } else {
            return (
              !self.is_clean && self.category_valid && self.isImagesValid()
            );
          }
        }
      };
      self.isImagesValid = function () {
        for (var i = 0; i < self.images.length; i++) {
          if (
            self.images[i].file != null &&
            self.images[i].valid != null &&
            !self.images[i].valid
          )
            return false;
        }
      ...
      self.uploadOptionalImages = function (p_id, n, idx) {
        if (n < self.images.length) {
          var nfile = null;
          if (self.images[n] && self.images[n].file) {
            nfile = self.images[n].file;
          }
          if (nfile && self.images[n].valid) {
            var name =
              n + '_' + new Date().getTime() + root.getExtension(nfile);
            var oldname =
              self.images_obj[n] != null && self.images_obj[n].name != null
                ? self.images_obj[n].name
                : '';
            request
              .uploadFileToUrl(nfile, dir, name, oldname)
              .then(function (resp) {
                if (resp.status == 'success') {
          ...
          for (var i = 0; i < 4; i++) {
            if (
              _index < self.images_obj.length &&
              self.images_obj[_index] == null
            ) {
          ...
          if (self.images_obj.length > 0) {
            request
              .insertAllProductImage(self.images_obj)
              .then(function (resp) {
      ...
      self.sendNotification = function (obj) {
        ...
        var body = root.getNotificationBody(
          'PRODUCT',
          obj,
          title,
          obj.name,
      ...
      self.viewImage = function (ev, f) {
        $mdDialog.show({
          ...
          file_url: f,
          template:
            '<md-dialog ng-cloak aria-label="viewImage">' +
                ...
                $mdToast
                  .show(
                    $mdToast
                      .simple()
                      .hideDelay(1000)
                      .content('Delete this image Success!')
                ...
                $mdToast
                  .show(
                    $mdToast
                      ...
                      .hideDelay(6000)
                      .action('CLOSE')
                      .content('Opps , Failed delete this image')
                      .position('bottom right')
                  )
